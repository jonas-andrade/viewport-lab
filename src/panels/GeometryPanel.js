...
export class GeometryPanel {
    ...
    _build() {
        ...
        }).on('change', ({ value }) => { this._swapGeometry(value) })

        f.addBinding(this.state, 'segments', {
            label: 'Segments', min: 1, max: 8, step: 1
        }).on('change', () => { this._swapGeometry(this.state.geometry) })

        f.addBlade({ view: 'separator' })

        f.addBinding(this.state, 'wireframe', {
            label: 'Wireframe'
        }).on('change', ({ value }) => { this.mesh.material.wireframe = value })
        ...
        }).on('change', ({ value }) => { this.mesh.scale.setScalar(value) })
    }
    ...
    _swapGeometry(name) {
        ...
        let geo
        switch (name) {
            case 'Box':        geo = new THREE.BoxGeometry(1.5, 1.5, 1.5, seg, seg, seg); break
            case 'Sphere':     geo = new THREE.SphereGeometry(0.9, 8 * seg, 8 * seg); break
            case 'Torus':      geo = new THREE.TorusGeometry(0.7, 0.3, 4 * seg, 12 * seg); break
            case 'Cone':       geo = new THREE.ConeGeometry(0.9, 1.8, 8 * seg, seg); break
            case 'Cylinder':   geo = new THREE.CylinderGeometry(0.7, 0.7, 1.8, 8 * seg, seg); break
            case 'Plane':      geo = new THREE.PlaneGeometry(2, 2, seg, seg); break
            case 'Capsule':    geo = new THREE.CapsuleGeometry(0.5, 1, 2 * seg, 8 * seg); break
            case 'TorusKnot':  geo = new THREE.TorusKnotGeometry(0.6, 0.2, 32 * seg, 8 * seg); break
            case 'Icosahedron': geo = new THREE.IcosahedronGeometry(1, seg - 1); break
            default: geo = GEOMETRIES[name]?.() ?? new THREE.BoxGeometry(1.5, 1.5, 1.5)
        }
